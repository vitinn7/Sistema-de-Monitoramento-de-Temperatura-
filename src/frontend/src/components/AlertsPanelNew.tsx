...
export const AlertsPanel: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'recent' | 'configs' | 'stats'>('recent');
  ...
  const loadAlertsData = async () => {
    try {
      ...
      const [recentResponse, configsResponse, statsResponse] = await Promise.all([
        apiClient.get('/alertas/recentes?limit=20'),
        apiClient.get('/alertas/configs'),
        apiClient.get('/alertas/estatisticas')
      ]);
